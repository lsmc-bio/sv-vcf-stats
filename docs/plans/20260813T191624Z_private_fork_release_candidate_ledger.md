...
| FIX-003 | Redistribution | Record a terminal per-fixture public-release-candidate disposition without weakening the later publication gate | SUCCESS | legitimate_safety_handling | Gate 3 | orchestrator | Every primary and derived manifest entry carries `reviewed-public-release-candidate-2026-08-13`; `test_data/redistribution-review.json` binds that status to the RFC 8785 digest of every other manifest field |  | The builder defaults to pending and can apply the policy only to an exact digest match. |

...
| TEST-004 | Validation | Run targeted pytest, Ruff touched files, mypy package, and no broad local suite | SUCCESS | contract_test | Gate 2 | Lead | Integrated targeted pytest -> 22 passed in 3.10s; Ruff touched files passed; mypy 25 source files passed; no broad suite run |  | Focused local validation is green. |
...
| BENCH-001 | Benchmark | Run bounded indexed one-million-record 24-contig gVCF benchmark with two warm repetitions; enforce speed, thread, digest, and temp bounds | IN_PROGRESS | contract_test | Gate 2 | Evidence/release agent | Bugfix commit 46a64f4 proves coordinate-sorted indexed raw ID-dot NON_REF blocks; combined generator/distribution run -> 5 passed; candidate measurement still pending |  |  |
| PR-001 | Integration | Integrate disjoint patches into the clean release branch | SUCCESS | feature_implementation | Gate 3 | Lead | EventStore fdf092f, evidence dfecd83/34382d3/46a64f4, and IO c726201 cherry-picked without conflict |  | All disjoint agent slices are on the clean release branch. |
...
Status counts: SUCCESS 14; OPEN 10; IN_PROGRESS 1; ATTEMPTING_BUGFIX 0; DUPLICATE 0; NO_LONGER_NEEDED 0; FAIL 0; BLOCKED 0.
